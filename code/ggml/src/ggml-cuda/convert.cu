...
template<typename dst_t>
static __global__ void dequantize_block_q4_hqq(const void * __restrict__ vx, dst_t * __restrict__ yy, int nb32) {
    ...
    const block_q4_hqq * x = (const block_q4_hqq *)vx + ib;
    const float scale = __half2float(x->scale);
    const float zero = __half2float(x->zero);
    const float dm = -zero*scale;

    const uint8_t * q = x->qs + 4*il;

    for (int l = 0; l < 4; ++l) {
        y[l+ 0] = scale * (q[l] & 0xF) + dm;
        y[l+16] = scale * (q[l] >>  4) + dm;
    }
}
template<typename dst_t>
...
template<typename dst_t>
static void dequantize_row_q4_hqq_cuda(const void * vx, dst_t * y, const int64_t k, cudaStream_t stream) {
    const int nb32 = k / 32;
    const int nb = (k + 255) / 256;
    dequantize_block_q4_hqq<<<nb, 32, 0, stream>>>(vx, y, nb32);
}
...
to_fp16_cuda_t ggml_get_to_fp16_cuda(ggml_type type) {
    switch (type) {
        ...
        case GGML_TYPE_Q4_HQQ:
            return dequantize_row_q4_hqq_cuda;
        case GGML_TYPE_Q4_1:
