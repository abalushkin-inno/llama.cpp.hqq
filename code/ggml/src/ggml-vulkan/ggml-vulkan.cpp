...
static bool ggml_vk_matmul_int_shmem_support(const vk_device& device, const std::vector<uint32_t>& warptile, bool mul_mat_id, ggml_type src0_type) {
    ...
    switch (src0_type) {
        ...
        case GGML_TYPE_Q4_HQQ:    block_a_size = std430_size({{16, 4}, {2*fp_size,  fp_align}});                  break; // qs[16/4] + scale + zero
...
static bool ggml_vk_fa_scalar_uses_mmq(const vk_device& device, ggml_type k_type) {
    ...
    return device->integer_dot_product && device->subgroup_clustered &&
           (k_type == GGML_TYPE_Q4_0 || k_type == GGML_TYPE_Q4_HQQ || k_type == GGML_TYPE_Q4_1 ||
...
#define CREATE_MM2(TYPE, PIPELINE_NAME, NAMELC, WG_DENOMS, WARPTILE, PUSHCONST, PARAMCOUNT, ID) \
        ...
        if (device->coopmat_acc_f16_support) {
            ...
            CREATE_MM(GGML_TYPE_Q4_HQQ, pipeline_dequant_mul_mat_mat[GGML_TYPE_Q4_HQQ].f32acc, matmul_q4_hqq_f32, , mmq_wg_denoms, warptile_mmq, vk_mat_mat_push_constants, 3, );
...
#define CREATE_MM2(TYPE, PIPELINE_NAME, NAMELC, WG_DENOMS, WARPTILE, PUSHCONST, PARAMCOUNT, ID, REQSUBGROUPSIZE) \
        ...
        if (device->integer_dot_product) {
            ...
            CREATE_MMQ(GGML_TYPE_Q4_HQQ, pipeline_dequant_mul_mat_mat_q8_1[GGML_TYPE_Q4_HQQ], matmul_q4_hqq_q8_1, mmq_wg_denoms, warptile_mmq_int, vk_mat_mat_push_constants, 3, , 0);
        ...
        if (device->subgroup_ballot && device->subgroup_require_full_support && subgroup_min_size_16) {
            ...
            if (device->integer_dot_product) {
                ...
                CREATE_MMQ(GGML_TYPE_Q4_HQQ, pipeline_dequant_mul_mat_mat_id_q8_1[GGML_TYPE_Q4_HQQ], matmul_id_subgroup_q4_hqq_q8_1, mmq_wg_denoms, warptile_mmqid_int,   vk_mat_mat_id_push_constants, mul_mat_id_param_count, _id, mul_mat_subgroup_size);
            ...
            if (device->integer_dot_product) {
                ...
                CREATE_MMQ(GGML_TYPE_Q4_HQQ, pipeline_dequant_mul_mat_mat_id_q8_1[GGML_TYPE_Q4_HQQ], matmul_id_q4_hqq_q8_1, mmq_wg_denoms, warptile_mmqid_int,   vk_mat_mat_id_push_constants, mul_mat_id_param_count, _id, 0);
...
#define CREATE_MMQ(TYPE, PIPELINE_NAME, NAMELC, WG_DENOMS, WARPTILE, PUSHCONST, PARAMCOUNT, ID) \
        ...
        if (device->integer_dot_product) {
            ...
            CREATE_MMQ(GGML_TYPE_Q4_HQQ, pipeline_dequant_mul_mat_mat_q8_1[GGML_TYPE_Q4_HQQ].f32acc, matmul_q4_hqq_q8_1, mmq_wg_denoms, warptile_mmq_int, vk_mat_mat_push_constants, 3, );
    ...
    for (uint32_t w = 0; w < DMMV_WG_SIZE_COUNT; ++w) {
        ...
        for (uint32_t i = 0; i < mul_mat_vec_max_cols; ++i) {
            ...
            if (device->integer_dot_product) {
                ...
                ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_q8_1_f32[w][GGML_TYPE_Q4_HQQ][i], "mul_mat_vec_q4_hqq_q8_1_f32", arr_dmmv_q4_hqq_q8_1_f32_len[reduc], arr_dmmv_q4_hqq_q8_1_f32_data[reduc], "main", mul_mat_vec_num_bindings, sizeof(vk_mat_vec_push_constants), {1*rm_stdq_int, 1, 1}, {wg_size_subgroup_int, 1*rm_stdq_int, i+1}, 1, true, use_subgroups, subgroup_size_int);
        ...
        if (device->integer_dot_product) {
            ...
            ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_id_q8_1_f32[w][GGML_TYPE_Q4_HQQ], "mul_mat_vec_id_q4_hqq_q8_1_f32", arr_dmmv_id_q4_hqq_q8_1_f32_len[reduc], arr_dmmv_id_q4_hqq_q8_1_f32_data[reduc], "main", mul_mat_vec_id_num_bindings, sizeof(vk_mat_vec_id_push_constants), {1*rm_stdq_int, 1, 1}, {wg_size_subgroup_int, 1*rm_stdq_int}, 1, true, use_subgroups, subgroup_size_int);
...
#define SET_ROWS(itype) \
        ...
        ggml_vk_create_pipeline(device, device->pipeline_set_rows ## itype [GGML_TYPE_Q4_HQQ], "set_rows_q4_hqq" #itype, set_rows_q4_hqq ## itype ## _len, set_rows_q4_hqq ## itype ## _data, "main", 3, sizeof(vk_op_binary_push_constants), {1, 1, 1}, {1}, 1, true); \
