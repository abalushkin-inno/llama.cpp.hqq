...
vec2 dequantize(uint ib, uint iqs, uint a_offset) {
    const uint vui = uint(data_a[a_offset + ib].qs[iqs]);
    const float zero = data_a[a_offset + ib].zero;
    const float scale = data_a[a_offset + ib].scale;
    return (vec2(vui & 0xF, vui >> 4) - zero/scale);
}
vec4 dequantize4(uint ib, uint iqs, uint a_offset) {
    const uint vui = uint(data_a_packed16[a_offset + ib].qs[iqs/2]);
    const float zero = data_a[a_offset + ib].zero;
    const float scale = data_a[a_offset + ib].scale;
    return (vec4(vui & 0xF, (vui >> 4) & 0xF, (vui >> 8) & 0xF, vui >> 12) - zero/scale);
}
#endif
...
#if defined(DATA_A_Q4_HQQ)
vec2 get_scale(uint ib, uint a_offset) {
    return vec2(float(data_a[a_offset + ib].scale), 0);
}

vec2 get_zero(uint ib, uint a_offset) {
    return vec2(float(data_a[a_offset + ib].zero), 0);
}
#endif
